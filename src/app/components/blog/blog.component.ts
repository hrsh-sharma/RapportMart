...
export class BlogComponent {
  ...
  constructor(private store: Store, private route: ActivatedRoute,
    public blogService: BlogService, private seoService: SeoService) {
    this.route.queryParams.subscribe(params => {
      ...
      const title = this.filter.category ? `Blogs: ${this.filter.category.replaceAll('-', ' ')} | Rapportmart` :
                   this.filter.tag ? `Blogs: ${this.filter.tag.replaceAll('-', ' ')} | Rapportmart` : 'Our Latest Blogs | Rapportmart';
      
      this.seoService.setSEOData({
        title: title,
        description: 'Stay updated with the latest fashion trends, style tips, and news from Rapportmart. Explore our blog collection for your style inspiration.',
        url: 'https://rapportmart.com/blogs',
        canonicalUrl: 'https://rapportmart.com/blogs',
        type: 'website'
      });

      this.seoService.setCollectionPageStructuredData(title, 'Rapportmart Fashion & Lifestyle Blog', 'https://rapportmart.com/blogs');
