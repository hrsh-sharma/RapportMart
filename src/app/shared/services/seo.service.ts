...
export class SeoService {
  ...
  setCanonicalUrl(url: string): void {
    ...
    const existingTags = this.document.querySelectorAll('link[rel="canonical"]');
  ...
  setStructuredData(data: any): void {
    // We'll append structured data. Search engines handle multiple JSON-LD blocks.
  ...
  setProductStructuredData(product: any): void {
    ...
    const structuredData: any = {
      ...
      "brand": {
        "@type": "Brand",
        "name": product.brand?.name || "Rapportmart"
      },
      "offers": {
        "@type": "Offer",
        ...
        // First-party seller (the store itself). Identifies Rapportmart as the
        // direct seller of its own products — NOT a third-party marketplace listing.
        "seller": {
          "@type": "Organization",
          "name": "RAPPORTMART ECOMMERCE PRIVATE LIMITED"
        }
      }
    };
  ...
  setOrganizationStructuredData(): void {
    const structuredData = {
      ...
      "@type": "Organization",
      "name": "RAPPORTMART ECOMMERCE PRIVATE LIMITED",
      "legalName": "RAPPORTMART ECOMMERCE PRIVATE LIMITED",
      "url": "https://rapportmart.com",
      "logo": "https://rapportmart.com/assets/images/logo.png",
      "email": "info@rapportmart.com",
      "sameAs": [
        "https://www.instagram.com/rapportmart_official",
        "https://www.facebook.com/rapportmart",
        "https://x.com/rapportmart"
      ],
      "address": {
        "@type": "PostalAddress",
  ...
  /**
   * Set WebSite structured data (for search box)
   */
  setWebSiteStructuredData(): void {
    const structuredData = {
      ...
      "potentialAction": {
        "@type": "SearchAction",
        "target": "https://rapportmart.com/search?q={search_term_string}",
  ...
  /**
   * Set CollectionPage structured data
   */
  setCollectionPageStructuredData(name: string, description: string, url: string): void {
    const structuredData = {
      "@context": "https://schema.org",
      "@type": "CollectionPage",
      "name": name,
  ...
  setArticleStructuredData(article: any): void {
    const structuredData = {
      ...
      "publisher": {
        ...
        "logo": {
          "@type": "ImageObject",
          "url": "https://rapportmart.com/assets/images/logo.png"
        }
      },
  ...
  updateDefaultSeo(): void {
    ...
    console.log('✅ Setting default SEO');
    this.clearStructuredData();
    this.setSEOData({
      title: 'Stylish Clothes Online for Men & Women | Rapportmart ',
      description: 'Shop the latest fashion trends at Rapportmart. Discover affordable clothing for men and women including t-shirts, dresses, hoodies & more. Shop now.',
      keywords: 'activewear, gym wear, joggers, men\'s clothes, women\'s clothes, stylish outfits, comfort fit, performance clothing, rapportmart',
      type: 'website',
      url: 'https://rapportmart.com'
    });
   ...
   */
  setProductPageSEO(product: any, productSlug: string | null, baseUrl: string = 'https://rapportmart.com'): void {
  ...
  private generateProductTitle(product: any): string {
    ...
    // Example: "Nike Air Max 270 Men's Running Shoes - ₹8,999 | Rapportmart"
    return `${brand}${product.name}${category} - ${price} | Rapportmart`;
  }
  ...
  private generateProductDescription(product: any): string {
    ...
    let rawDescription: string = product.meta_description
      ...
      || product.description
      || `Shop ${brand}${product.name}${category} online at Rapportmart. Premium quality, great prices, fast delivery.${discount}`;
  ...
  private sanitizeProductDescription(text: string): string {
    ...
    if (!sanitized) {
      sanitized = 'Premium quality, great prices, fast delivery from Rapportmart.';
    }
  ...
  private generateProductKeywords(product: any): string {
    ...
    keywords.push('buy online', 'Rapportmart', 'fashion', 'clothing');
